...
/* Служебная информация LCD */
static struct str_lcd
{
			uint8_t page;
...
static uint8_t bufferLCD[HightLCD / 8][WidthLCD] = { 0x00 }; /* Буффер, где храниться изображение (Размером 122 на 32 px) */
...
static void delay( uint32_t mks )
{
	uint32_t coin = mks * 1;
	while(coin--);
}

__STATIC_INLINE void A0( uint8_t mode )		{ ( mode == 1 ) ? PORT_SetBits( LCD_CONTROL_PORT, LCD_A0 )  : PORT_ResetBits( LCD_CONTROL_PORT, LCD_A0 ); }
__STATIC_INLINE void RW( uint8_t mode )		{ ( mode == 1 ) ? PORT_SetBits( LCD_CONTROL_PORT, LCD_RW )  : PORT_ResetBits( LCD_CONTROL_PORT, LCD_RW ); }
__STATIC_INLINE void E( uint8_t mode )		{ ( mode == 1 ) ? PORT_SetBits( LCD_CONTROL_PORT, LCD_E ) 	: PORT_ResetBits( LCD_CONTROL_PORT, LCD_E ); } 
__STATIC_INLINE void RES( uint8_t mode )	{ ( mode == 1 ) ? PORT_SetBits( LCD_CONTROL_PORT, LCD_RES ) : PORT_ResetBits( LCD_CONTROL_PORT, LCD_RES ); }
__STATIC_INLINE void CS( uint8_t mode )		{ ( mode == 1 ) ? PORT_SetBits( LCD_CONTROL_PORT, LCD_CS )  : PORT_ResetBits( LCD_CONTROL_PORT, LCD_CS ); }
__STATIC_INLINE void SetByteData( uint8_t data )		{ PORT_Write( LCD_DATA_PORT, (uint32_t)( data << 8 ) ); }
__STATIC_INLINE void SetByteCommand( uint8_t data ) { PORT_Write( LCD_DATA_PORT, (uint32_t)( data << 8 ) ); }

static void WriteByteData( uint8_t byte, uint8_t bit_A0, uint8_t bit_CS )
{// Запись байта 	данных
	taskENTER_CRITICAL();
	{
		RW(0);
		...
		CS(bit_CS);
		SetByteData(byte);
		delay(1); //Это время предустановки адреса (tAW) 100ns
		E(0);
		delay(1); // Время предустановки данных попало сюда (tDS)) 160ns
		E(1);
		delay(10); // Минимально допустимый интервал между сигналами 2000ns-100ns-160ns
	}
	taskEXIT_CRITICAL();
}

static void WriteByteCommand( uint8_t byte, uint8_t bit_A0, uint8_t bit_CS )
{// Запись байта	комманды
	taskENTER_CRITICAL();	
	{
		RW(0);
		A0(bit_A0);
...
__STATIC_INLINE void WRITE_COMMANDL( uint8_t byte ){ WriteByteCommand( byte, 0, 1 ); } // Команду в левый кристалл индикатора
__STATIC_INLINE void WRITE_COMMANDR( uint8_t byte ){ WriteByteCommand( byte, 0, 0 ); } // Команду в правый кристалл индикатора

__STATIC_INLINE void WRITE_DATAL( uint8_t byte ){ WriteByteData( byte, 1, 1 ); } // Данные в левую половину индикатора
__STATIC_INLINE void WRITE_DATAR( uint8_t byte ){ WriteByteData( byte, 1, 0 ); } // Данные в правую половину индикатора

void LCD_InitGPIO( void )
{
	PORT_InitTypeDef PORT_InitStructure;
	
	RST_CLK_PCLKcmd( (LCD_CONTROL_PCLK_PORT | LCD_DATA_PCLK_PORT), ENABLE);
	
	PORT_StructInit(&PORT_InitStructure);
	PORT_DeInit(LCD_DATA_PORT);
	
	PORT_InitStructure.PORT_Pin = ( LCD_DB0 | LCD_DB1 | LCD_DB2 | LCD_DB3 | LCD_DB4 | LCD_DB5 | LCD_DB6 | LCD_DB7 );
	PORT_InitStructure.PORT_OE = PORT_OE_OUT;
	PORT_InitStructure.PORT_FUNC = PORT_FUNC_PORT;
	PORT_InitStructure.PORT_MODE = PORT_MODE_DIGITAL;
	PORT_InitStructure.PORT_SPEED = PORT_SPEED_MAXFAST;
	PORT_Init(LCD_DATA_PORT, &PORT_InitStructure);
	
	PORT_DeInit(LCD_CONTROL_PORT);
	PORT_InitStructure.PORT_Pin = ( LCD_A0 | LCD_RW | LCD_E | LCD_RES | LCD_CS );
	PORT_InitStructure.PORT_OE = PORT_OE_OUT;
	PORT_InitStructure.PORT_FUNC = PORT_FUNC_PORT;
	PORT_InitStructure.PORT_MODE = PORT_MODE_DIGITAL;
	PORT_InitStructure.PORT_SPEED = PORT_SPEED_MAXFAST;
	PORT_Init(LCD_CONTROL_PORT, &PORT_InitStructure);	
}
...
void LCD_Init( void )
{
	...
	E(1);
	RES(0);
	delay(10); 		// Задержка не менее 10 мкс
	RES(1);
	delay(1000); 	// Задержка не менее 2 мс
	WRITE_COMMANDL(0xE2); // Reset
	WRITE_COMMANDR(0xE2); // Reset
	WRITE_COMMANDL(0xEE); // Cнятия флага RMW (END);
	WRITE_COMMANDR(0xEE); // Cнятия флага RMW (END);
	WRITE_COMMANDL(0xA4); // Включение обычного режима
	WRITE_COMMANDR(0xA4); // Включение обычного режима
	WRITE_COMMANDL(0xA9); // Выбор мультиплекса (Duty Select); Мультиплекс 1/32
	WRITE_COMMANDR(0xA9); // Выбор мультиплекса (Duty Select); Мультиплекс 1/32
	WRITE_COMMANDL(0xAF); // Display on
	WRITE_COMMANDR(0xAF); // Display on	
	WRITE_COMMANDL(0xC0); // Верхнюю строку на 0
	WRITE_COMMANDR(0xC0); // Верхнюю строку на 0
	WRITE_COMMANDL(ADC_L); // Invert scan RAM обратное отображение (ADC=1 на левом кристале)
	WRITE_COMMANDR(ADC_R); // NonInvert scan RAM прямое отображение (ADC=0 на правом кристале)
}

void LCD_SetCursor( uint8_t posCursor, uint8_t page )
{
	if( xSemaphoreTakeRecursive( xRecursiveMutexBufferLCD, portMAX_DELAY ) == pdTRUE ) 
	{		
		str_lcd_t.positionCursor = ( posCursor > 20 ) ? str_lcd_t.positionCursor : posCursor;	
		str_lcd_t.page = ( page > 3 ) ? str_lcd_t.page : page;	

    xSemaphoreGiveRecursive( xRecursiveMutexBufferLCD );
	}	
}
...
void LCD_SetPositionCursor( uint8_t posCursor )
{	
	if( xSemaphoreTakeRecursive( xRecursiveMutexBufferLCD, portMAX_DELAY ) == pdTRUE )
	{
		str_lcd_t.positionCursor = ( posCursor > 20 ) ? str_lcd_t.positionCursor : posCursor;
...
{
	return str_lcd_t.page;
}

uint8_t LCD_GetPositionCursor( void )
{	
	return str_lcd_t.positionCursor;
}

void LCD_DisplayFill( uint8_t byte )
{
	uint8_t	page, column;
	
	if( xSemaphoreTakeRecursive( xRecursiveMutexBufferLCD, portMAX_DELAY ) == pdTRUE ) 
	{
		for(page = 0; page < HightLCD/8; page++)
		{				  
			for (column = 0; column <= WidthLCD/2; column++)
			{	
				bufferLCD[page][column] = byte;
				bufferLCD[page][62 + column] = byte;
			}
		}
...
void LCD_ClearSumbol( uint8_t posCursor, uint8_t page )
{
	uint8_t i;

	if(xSemaphoreTakeRecursive( xRecursiveMutexBufferLCD, portMAX_DELAY ) == pdTRUE ) 
	{	
		for(i = 0; i < 6; i++) /* Цикл вывода элем. на экран */
		{	
			bufferLCD[page][posCursor * 6 + i] = 0x00;
		}
...
void LCD_ClearPage( uint8_t page )
{
	...
	if( xSemaphoreTakeRecursive( xRecursiveMutexBufferLCD, portMAX_DELAY ) == pdTRUE ) 
	{
		for(column = 0; column <= WidthLCD; column++) /* Цикл перебора всех столбцов на левом и правом кристале */
		{	
			bufferLCD[page][column] = 0x00;
		}
...
void LCD_WriteSymbol( char sumbol )
{
	uint8_t i;

	if( xSemaphoreTakeRecursive( xRecursiveMutexBufferLCD, portMAX_DELAY ) == pdTRUE ) 
	{	
		for(i = 0; i < 6; i++) /* Цикл вывода байтов символа на экран */
		{
			bufferLCD[str_lcd_t.page][str_lcd_t.positionCursor * 6 + i] = ( (uint8_t) LIBRARY_SYMBOL[ (sumbol * 6) + i ] ) + str_lcd_t.cursorBlink;
		}
...
void LCD_WriteString( const char*  str )
{
	...
	if( xSemaphoreTakeRecursive( xRecursiveMutexBufferLCD, portMAX_DELAY ) == pdTRUE ) 
	{	
		for(j = 0; str[j] != 0 && ( str_lcd_t.positionCursor < 20 ); j++, str_lcd_t.positionCursor++)
		{
			for(i = 0; i < 6; i++) /* Цикл вывода байтов символа на экран */
			{
				bufferLCD[str_lcd_t.page][str_lcd_t.positionCursor * 6 + i] = ( (uint8_t) LIBRARY_SYMBOL[ (str[j] * 6) + i ] ) + str_lcd_t.cursorBlink;
			}
		}	
...
void LCD_DrawHorizontalLine( uint8_t y )
{
	uint32_t x;
	uint8_t yLine, line;
	...
	if( y >= HightLCD ) 
	{
		return; /* Выход за пределы индикатора */
	}
	if( xSemaphoreTakeRecursive( xRecursiveMutexBufferLCD, portMAX_DELAY ) == pdTRUE ) 
	{			
		for(yLine	= 0; y > 7; yLine++)
		{
			y = y - 8;
		}
		line = 0x01 << y;
		for(x = 0; x < WidthLCD; x++)
		{
			bufferLCD[yLine][x] |= line;
		}
...
void LCD_DrawVerticalLine( uint8_t x )
{
	uint32_t y;
...
void LCD_DrawDisplay(void) // Рисует значения из буфера
{
	uint8_t byte = 0x00; 
	uint8_t page, column;
	...
	if( xSemaphoreTakeRecursive( xRecursiveMutexBufferLCD, portMAX_DELAY ) == pdTRUE ) 
	{		
		for(page = 0; page < HightLCD/8; page++)//Цикл по всем 4-м страницам индикатора
		{
			WRITE_COMMANDL( page | 0xB8 );//Установка текущей страницы для левого кристалла индикатора
			WRITE_COMMANDL( 0x13 );//Установка текущего адреса для записи данных в левую отображаемую позицию левой половины индикатора
			WRITE_COMMANDR( page | 0xB8 );//Установка текущей страницы для правого кристалла индикатора
			WRITE_COMMANDR( 0x00 );//Установка текущего адреса для записи данных в правую отображаемую позицию правой половины индикатора
			for (column = 0; column <= WidthLCD/2; column++)//Цикл вывода данных в индикатор
			{
				#ifndef	USE_MODE_ADC_SELECT
					byte = bufferLCD[page][column + 0];
					WRITE_COMMANDL( column + 0x13 );
					WRITE_DATAL( byte );//Вывод очередного байта в индикатор

					byte = bufferLCD[page][column + 61];
					WRITE_COMMANDR( column + 0x00 );
					WRITE_DATAR( byte );//Вывод очередного байта в индикатор
				#else
					byte = bufferLCD[3 - page][column + 0];
					WRITE_COMMANDR( column + 0x13 );
					WRITE_DATAR( byte );//Вывод очередного байта в индикатор

					byte = bufferLCD[3 - page][column + 61];
					WRITE_COMMANDL( column + 0x00 );
					WRITE_DATAL( byte );//Вывод очередного байта в индикатор			
				#endif
			}
		}
...
{
	str_lcd_t.cursorBlink = 0x80;
}
void LCD_CursorOff(void)
{
	str_lcd_t.cursorBlink = 0x00;
}
...
void LCD_RewriteSymbol( uint8_t posCursor, uint8_t page )
{
	uint8_t i;
	...
	{	
		for(i = 0; i < 6; i++) /* Цикл вывода байтов символа на экран */
		{
			bufferLCD[page][posCursor * 6 + i] = bufferLCD[page][posCursor * 6 + i] + str_lcd_t.cursorBlink;
		}
